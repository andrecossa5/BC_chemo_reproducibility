...
def fastGSEA(s, collection='MSigDB_Hallmark_2020', n_top=50):  #GO_Biological_Process_2023
    """
...
already_done= ['1_proCTC_rest','2_proCTC_PT_rest','pro_CTC_all','3_proCTC_PT_rest','4_proCTC_rest','pt_ctc',
               'lung_ctc','pt_lung','ctc_pt/lung']
for i, df in enumerate(dfs):
    if df['label'].iloc[0] == '1_proCTC_rest':
        try:
            ...
            df_.to_csv(
                os.path.join(
                    path_results, 'GSEA', 'Hallmark','ctc_pt_lung_g1_vs_g0'
                )
            )
            time.sleep(2)
