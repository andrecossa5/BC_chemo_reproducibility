...
for infection in df['infection'].unique():
    ...
    for dataset in df_['dataset'].unique():
        ...
        df_promet = df_gbc_sample.loc[:, pd.IndexSlice['count', samples_names]].droplevel(0, axis=1)
        ...
        packed_circle_plot(df_promet, covariate='PT_prevalence', ax=axs[0], color='orange', annotate=True)
        axs[0].set(title='PT prevalence')
        packed_circle_plot(df_promet, covariate='lung_prevalence', ax=axs[1], color='r', annotate=True)
